locals {
  aws_region                            = "us-east-1"
  account_id                            = "311923415472"
  repository                            = "wearecircleup/wearecircleup-infra"
  github_actions_role                   = "arn:aws:iam::311923415472:role/GitHubActionsDeployRole"
  state_bucket_name                     = "wearecircleup-terraform-state-311923415472-us-east-1"
  validation_bucket_name                = "wearecircleup-terraform-check-311923415472-us-east-1"
  public_assets_bucket_name             = "wearecircleup-prod-public-assets-311923415472-us-east-1"
  eventbrite_secret_name                = "wearecircleup/prod/eventbrite"
  ses_domain                            = "circleup.com.co"
  eventbrite_api_name                   = "wearecircleup-prod-eventbrite-api"
  eventbrite_api_lambda                 = "wearecircleup-prod-eventbrite-api"
  eventbrite_api_role                   = "wearecircleup-prod-eventbrite-api-role"
  eventbrite_order_webhook_api          = "wearecircleup-prod-eventbrite-order-webhook"
  eventbrite_order_webhook_lambda       = "wearecircleup-prod-eventbrite-order-webhook"
  eventbrite_order_webhook_role         = "wearecircleup-prod-eventbrite-order-webhook-role"
  eventbrite_order_webhook_path         = "/webhooks/eventbrite/order-place"
  eventbrite_order_webhook_table        = "wearecircleup-prod-eventbrite-order-submissions"
  minor_authorization_jobs_table        = "wearecircleup-prod-minor-authorization-jobs"
  minor_authorization_reminder_lambda   = "wearecircleup-prod-minor-authorization-reminder"
  minor_authorization_reminder_role     = "wearecircleup-prod-minor-authorization-reminder-role"
  minor_authorization_reminder_rule     = "wearecircleup-prod-minor-authorization-reminder-daily"
  minor_authorization_reminder_sender   = "Circle Up Autorizacion <hola@circleup.com.co>"
  minor_authorization_reminder_form     = "https://app.youform.com/forms/iamr7tnj"
  minor_authorization_reminder_hero     = "https://wearecircleup-prod-public-assets-311923415472-us-east-1.s3.us-east-1.amazonaws.com/email-assets/logo.png"
  minor_authorization_validator_lambda  = "wearecircleup-prod-minor-authorization-validator"
  minor_authorization_validator_role    = "wearecircleup-prod-minor-authorization-validator-role"
  minor_authorization_processor_lambda  = "wearecircleup-prod-minor-authorization-processor"
  minor_authorization_processor_role    = "wearecircleup-prod-minor-authorization-processor-role"
  minor_authorization_validation_queue  = "wearecircleup-prod-minor-authorization-validation"
  minor_authorization_validation_dlq    = "wearecircleup-prod-minor-authorization-validation-dlq"
  youform_webhook_api                   = "wearecircleup-prod-youform-webhook"
  youform_webhook_lambda                = "wearecircleup-prod-youform-webhook"
  youform_webhook_role                  = "wearecircleup-prod-youform-webhook-role"
  youform_webhook_path                  = "/webhooks/youform"
  youform_webhook_table                 = "wearecircleup-prod-youform-submissions"
  youform_volunteer_intent_table        = "wearecircleup-prod-youform-volunteer-intent-proposals"
  youform_volunteer_intent_sender       = "Circle Up Voluntariado <hola@circleup.com.co>"
  youform_volunteer_intent_recipient    = "wearecircleup@gmail.com,hola@circleup.com.co"
  youform_volunteer_intent_reply_to     = "hola@circleup.com.co"
  youform_volunteer_intent_logo         = "https://wearecircleup-prod-public-assets-311923415472-us-east-1.s3.us-east-1.amazonaws.com/email-assets/logo.png"
  volunteer_intent_notifier_lambda      = "wearecircleup-prod-volunteer-intent-notifier"
  volunteer_intent_notifier_role        = "wearecircleup-prod-volunteer-intent-notifier-role"
  youform_background_check_table        = "wearecircleup-prod-youform-volunteer-background-checks"
  background_check_reviews_table        = "wearecircleup-prod-background-check-reviews"
  background_check_dispatcher_lambda    = "wearecircleup-prod-background-check-dispatcher"
  background_check_dispatcher_role      = "wearecircleup-prod-background-check-dispatcher-role"
  background_check_reviewer_lambda      = "wearecircleup-prod-background-check-reviewer"
  background_check_reviewer_role        = "wearecircleup-prod-background-check-reviewer-role"
  background_check_review_queue         = "wearecircleup-prod-background-check-review"
  background_check_review_dlq           = "wearecircleup-prod-background-check-review-dlq"
  background_check_notification_sender  = "Circle Up Verificacion <hola@circleup.com.co>"
  background_check_notification_to      = "wearecircleup@gmail.com,hola@circleup.com.co"
  background_check_notification_reply   = "hola@circleup.com.co"
  background_check_notification_logo    = "https://wearecircleup-prod-public-assets-311923415472-us-east-1.s3.us-east-1.amazonaws.com/email-assets/logo.png"
  background_check_internal_review_form = "https://app.youform.com/forms/p35vzbna"
  youform_signatures_bucket             = "wearecircleup-prod-youform-signatures-311923415472-us-east-1"
  youform_background_check_bucket       = "wearecircleup-prod-bg-check-files-311923415472-us-east-1"

  common_tags = {
    ManagedBy   = "terraform"
    Environment = "prod"
    Project     = "wearecircleup"
    Repository  = local.repository
  }
}
