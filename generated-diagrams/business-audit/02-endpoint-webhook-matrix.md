...
| `POST /webhooks/youform` for background compliance | YouForm | `youform_webhook` | Store submission, copy PDFs and enqueue reviews | Background submissions table, S3 | One SQS message per supported PDF | Classification is by question/key text and file extension |
...
| SQS `background_check_review` | YouForm webhook | `background_check_reviewer` | Review one PDF, persist result and maybe finalize summary | Reviews table | SES email to ops | Cedula uses Bedrock; certificates use Textract |
